...
const ReconcileGeneralAccount = () => {
  ...
  const [statementAmount, setStatementAmount] = useState(() => {
    ...
    const saved = localStorage.getItem(
      "reconcileGeneralAccount_statementAmount"
    );
    return saved || "";
  });
  ...
  const handleRetrieveData = async () => {
    ...
    setLoading(true);
    try {
      // Fetch ledger entries for account 10001 (Cash - Current Account) within the date range
      const response = await axiosInstance.get(`/ledger/account/10001`, {
        params: {
          fromDate: fromDate,
          toDate: toDate,
        },
  ...
  const handleStatementSubmit = async () => {
    try {
      ...
      await axiosInstance.put(
        `/reconciliation-settings/10001/statement-amount`,
        {
          fromDate: fromDate,
          toDate: toDate,
  ...
  const handleClearTransaction = async (shouldClear) => {
    if (selectedTransaction) {
      ...
      try {
        await axiosInstance.put(
          `/reconciliation-settings/10001/cleared-transactions`,
          {
            fromDate: fromDate,
            toDate: toDate,
                    ...
                    .map((transaction) => {
                      ...
                      if (payee === "-") {
                        ...
                        if (receivedFromMatch) {
                          ...
                          payee = payeeMatch ? payeeMatch[1].trim() : "-";
                        }
                      }

                      // Format payee based on transaction type
                      // Check if this should be a "Paid to" transaction based on address or description
                      const shouldBePaidTo =
                        address === "Ayara" ||
                        (transaction.description &&
                          transaction.description.includes("Paid to:"));

                      if (shouldBePaidTo) {
                        // For payments, show "Paid to:" with name from address field
                        payee = `Paid to: ${address}`;
                      } else {
                        // For other transactions, show "Received from:" with payee name
                        if (payee !== "-") {
                      ...
                      // Determine what to show in the Trade # column
                      let tradeDisplay = transaction.tradeNumber || "-";
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            {showTable && (
              <div className="bg-white rounded-lg shadow-md">
                <h3 className="text-lg font-semibold p-6 border-b border-gray-200">
                  Cash - Current Account Transactions (10001)
                </h3>
...
export default ReconcileGeneralAccount;
