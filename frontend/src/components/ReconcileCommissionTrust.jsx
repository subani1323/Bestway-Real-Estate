                ...
                <tbody>
                  ${unclearedTransactions
                    .map((transaction) => {
                      ...
                      // Extract payee from description field
                      let payee = "-";
                      if (transaction.description) {
                        ...
                        if (isDebitEntry) {
                          // For debit entries, prioritize "Received from" extraction
                          ...
                          if (receivedFromMatch) {
                            ...
                            // Fallback to "Paid to" if "Received from" not found
                            const paidToMatch =
                              transaction.description.match(/Paid to: ([^-]+)/);
                            if (paidToMatch) {
                              payee = paidToMatch[1].trim();
                            } else {
                              // Additional fallback: try to extract name from other patterns
                              // Look for patterns like "Trade #: XXX - Name - Address"
                              const nameMatch = transaction.description.match(
                                /Trade #:?\s*\d+\s*-\s*([^-]+?)\s*-\s*[^-]+$/
                              );
                              if (nameMatch) {
                          ...
                          // For credit entries, prioritize "Paid to" extraction
                          const paidToMatch =
                            transaction.description.match(/Paid to: ([^-]+)/);
                          if (paidToMatch) {
                            payee = paidToMatch[1].trim();
                          } else {
                            // Fallback to "Received from" if "Paid to" not found
                            const receivedFromMatch =
                              transaction.description.match(
                                /Received from: ([^-]+)/
                              );
                      ...
                      if (payee !== "-") {
                        // For Commission Trust account (10004):
                        // - Credit transactions = "Paid to:"
                        // - Debit transactions = "Received from:"
                        const isCreditEntry = transaction.type === "Credit";
                        const isDebitEntry = transaction.type === "Debit";
                      ...
                      let tradeDisplay = "-";
                      if (transaction.description) {
                        // Try to extract trade number from description
                        const tradeMatch =
                          transaction.description.match(/Trade #:?\s*(\d+)/);
                        if (tradeMatch) {
